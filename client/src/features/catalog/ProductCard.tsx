...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      href={`/p/${product.slug}`}
      className="press group flex h-full flex-col overflow-hidden rounded-2xl bg-surface shadow-card hover:shadow-hover"
    >
      {/* Media */}
      <div className="relative aspect-square w-full overflow-hidden bg-white">
        ...
        {hasDiscount ? (
          <span className="absolute left-2 top-2 inline-flex items-center rounded-lg bg-hot-500 px-2 py-0.5 text-[11px] font-bold text-white shadow-cta">
            −{discountPct}%
          </span>
        ) : null}

        {/* Favorite affordance — visual, matches food-app card chrome */}
        <span
          className="absolute right-2 top-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-surface/90 text-ink-muted shadow-card backdrop-blur transition-colors group-hover:text-hot-500"
          aria-hidden
        >
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        ...
        {offer ? (
          <span
            className="mt-auto inline-flex h-9 items-center justify-center gap-1.5 rounded-xl bg-gradient-premium pt-px text-[13px] font-semibold text-white shadow-cta"
            aria-hidden
          >
