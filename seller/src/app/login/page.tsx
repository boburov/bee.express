...
import { hasSellerRole, SELLER_ROLE_SLUG, useAuthStore } from "@/lib/auth-store";
  ...
  async function verifyOtp(e: FormEvent) {
    ...
    try {
      ...
      if (!hasSellerRole(me.data)) {
        ...
        clear();
        setError(`Bu hisob ${SELLER_ROLE_SLUG} roliga ega emas. Administrator bilan bog'laning.`);
        return;
      }
      setMe(me.data);
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="mb-8 flex flex-col items-center gap-3">
          <Logo size={44} />
          <p className="text-sm text-ink-muted">Sotuvchi paneli</p>
        </div>
        ...
        <p className="mt-6 text-center text-xs text-ink-muted">
          Faqat sotuvchi roli berilgan hisoblar kirishi mumkin. Hisob yo&apos;qmi? Avval{" "}
          <a
            href={`https://t.me/${BOT_USERNAME}`}
            target="_blank"
            rel="noopener noreferrer"
